...
<div id="scrollContent" >
	...
	<table class="tableStyle"  useCheckBox="true" sortMode="true" mode="list">
		<tr>
			<th width="25">序号</th>
			<th width="50"><span  id="span_userName">商品名</span></th>
			<th width="25"><span  id="span_userName">代号</span></th>
			<th width="30"></th>
		</tr>
		<c:forEach items="${goodList}" var="good" varStatus="status">
		    <tr>
				...
				<span class="img_delete hand" title="删除"  onclick='top.Dialog.confirm("确定要删除 __${good.name}__吗",function(){location="<%=path %>/good/delete?deleteId=${good.id}"});'></span>
				</td>
		    </tr>
		</c:forEach>
...
///添加
	function addUser() {
		...
		diag.Title = "增加";
		diag.URL = "<%=path%>/manage/good/add.jsp";
		diag.Width=400;
		diag.Height=150;
		diag.show();
